...
/** Confirmations before a deposit is shown as confirmed.
 *
 *  NON-CUSTODIAL MODEL (Trust Wallet style), not exchange-custody model.
 *  Trust Wallet does not hold user funds, so it does not wait out an
 *  exchange-grade reorg window: a transfer flips from "Pending" to "Completed"
 *  once it is "included in a block and confirmed by the network"
 *  (trustwallet.com/blog/academy/transaction-status-in-trust-wallet,
 *  trustwallet.com/glossary/block). Deep counts (12/20/128) are a custodial
 *  double-spend defence — wrong tool here, they just make a self-custody
 *  balance look broken for minutes.
 *
 *  So each chain waits for ITS OWN notion of inclusion/finality, not a
 *  padded block count:
 *    BTC 1  — no protocol finality at all; 1 conf = mined. 6 is the classic
 *             high-value merchant number (en.bitcoin.it/wiki/Confirmation),
 *             set DEPOSIT_MIN_CONF_BTC=3..6 for large-amount policies.
 *    XRP 1  — finality is the *validated ledger* flag, not a count
 *             (xrpl.org/docs/concepts/transactions/finality-of-results).
 *    ETH 1  — single-slot inclusion; "finalized" is ~12.8 min (2 epochs) and
 *             not something a wallet UI should block on.
 *    BSC 3  — fast finality lands in ~0.65s (BEP-126), so 3 blocks ~= final
 *             (bnbchain.org fast-finality).
 *    POLYGON 5 — Heimdall v2 "milestones" give deterministic finality in 2-5s
 *             (docs.polygon.technology/pos/concepts/finality). The old 128 was
 *             the pre-milestone checkpoint-era reorg window.
 *    ARBITRUM/OPTIMISM 1 — sequencer soft-confirmation; the only stronger
 *             guarantee is L1 batch finality / the 7-day fraud window, which no
 *             wallet waits for (docs.optimism.io/op-stack/transactions/transaction-finality).
 *    AVAX 1 — ~2s single-slot finality.
 *    SOL 32 — kept: this is the `finalized` commitment (supermajority + 31
 *             blocks) and the Solana lane uses it as its finalized sentinel.
 *             ~12.8s in practice (solana.com/docs/rpc).
 *    TRX 19 — Tron blocks are reversible until *solidified* by 2/3+1 SRs,
 *             ~19 blocks / ~57s (developers.tron.network/docs/concensus).
 */
const MIN_CONFIRMATIONS_DEFAULT: Record<ChainKey, number> = {
  BTC: 1,
  XRP: 1,
  ETH: 1,
  BSC: 3,
...
/** Per-chain override, e.g. DEPOSIT_MIN_CONF_BTC=1. Never below 1 —
 *  zero-conf money is not money (same rule Trust Wallet applies). */
const MIN_CONFIRMATIONS: Record<ChainKey, number> = Object.fromEntries(
  (Object.keys(MIN_CONFIRMATIONS_DEFAULT) as ChainKey[]).map((c) => {
    const raw = Number(Deno.env.get(`DEPOSIT_MIN_CONF_${c}`) ?? NaN)
    return [c, Number.isFinite(raw) && raw >= 1 ? Math.floor(raw) : MIN_CONFIRMATIONS_DEFAULT[c]]
  }),
...
interface Deposit {
  ...
  creditable: boolean
  /** true at >= 1 confirmation — safe to show in the wallet balance/UI. */
  confirmed: boolean
  /** Confirmations still needed before the deposit becomes creditable. */
...
function finalize(chain: ChainKey, txs: Deposit[]): Deposit[] {
  ...
  for (const t of txs) {
    ...
    t.confirmed = t.confirmations >= VISIBLE_CONFIRMATIONS
    t.confirmationsRemaining = Math.max(min - t.confirmations, 0)
    out.push(t)
  }
