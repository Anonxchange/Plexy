...
interface CartItem {
  ...
  price: number;
  currency: string;
  /** The Reloadly recipient/face value selected by the customer. */
...
export const GiftCardCartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = (itemData: Omit<CartItem, 'id' | 'quantity'>) => {
    setItems(prev => {
      ...
      if (existing) {
        return prev.map(i => i.productId === itemData.productId
          ? { ...i, ...itemData, quantity: i.quantity + 1 }
          : i);
      }
  ...
  return (
    <GiftCardCartContext.Provider value={{ items, isLoading, addToCart, updateItem, updateQuantity, removeItem, clearCart }}>
      {children}
