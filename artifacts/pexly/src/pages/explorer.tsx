    ...
    <section className="relative overflow-hidden rounded-b-[2.5rem] md:rounded-b-[3.5rem]" style={{ background: "linear-gradient(135deg, #0a0a0a 0%, #0f1208 50%, #0a1400 100%)" }}>
      ...
      <div className="relative z-10 container py-16 md:py-20">
        <div className="max-w-3xl mx-auto text-center animate-fade-in">
          ...
          <div className="relative max-w-2xl mx-auto flex items-center gap-1.5 md:gap-2 bg-white/10 backdrop-blur-md border border-white/20 rounded-lg md:rounded-xl p-1 md:p-1.5 shadow-2xl">
            <Search className="h-4 w-4 md:h-5 md:w-5 text-white/50 ml-2 flex-shrink-0" />
            <Input
              placeholder="Search BTC, ETH, SOL, BNB, TRON addresses or Txn Hash"
              className="flex-1 border-0 bg-transparent focus-visible:ring-0 text-white placeholder:text-white/40 h-8 md:h-auto text-sm md:text-base"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
            />
            <Button onClick={handleSearch} disabled={loading} className="px-5 md:px-8 rounded-md md:rounded-lg bg-primary hover:bg-primary/90 text-black font-semibold flex-shrink-0 h-8 md:h-10 text-sm md:text-base">
              {loading ? <Loader className="h-3.5 w-3.5 md:h-4 md:w-4 animate-spin" /> : 'Search'}
            </Button>
          </div>
