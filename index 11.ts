...
type EvmChain = 'ETH' | 'BSC' | 'POLYGON' | 'ARBITRUM' | 'OPTIMISM' | 'BASE' | 'AVAX'
...
const TOKENS: Record<ChainKey, Record<string, TokenDef>> = {
  ...
  BASE: {
    ...
    '0xfde4c96c8593536e31f229ea8f37b2ada2699bb2': { symbol: 'USDC', decimals: 6, label: 'USDbC' },
  },
  AVAX: {
...
const ALCHEMY_CHAINS = ['ETH', 'BSC', 'POLYGON', 'ARBITRUM', 'OPTIMISM', 'BASE', 'AVAX', 'SOL', 'TRX']
const CHAIN_KEYS: ChainKey[] = ['BTC', 'XRP', 'ETH', 'BSC', 'POLYGON', 'ARBITRUM', 'OPTIMISM', 'BASE', 'AVAX', 'SOL', 'TRX']
...
function normalizeChain(input: string): ChainKey | null {
  ...
  const key = ['BITCOIN', 'BTC'].includes(c) ? 'BTC'
    ...
    : ['BASE', 'BASE_MAINNET', 'BASECHAIN'].includes(c) ? 'BASE'
