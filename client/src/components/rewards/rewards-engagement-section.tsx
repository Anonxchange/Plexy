...
// ─── Task → page route mapping ────────────────────────────────────────────────

const TASK_ROUTE: Record<string, string> = {
  "daily-prediction": "/prediction",
  "daily-swap":       "/swap",
  "daily-spot":       "/spot",
  "daily-perpetual":  "/perpetual",
  "daily-giftcard":   "/gift-cards",
  "verify":           "/verification",
  "enable-2fa":       "/account-settings",
  "first-trade":      "/spot",
  "first-swap":       "/swap",
  "first-giftcard":   "/gift-cards",
  "refer":            "/referral",
...
function TaskDetailModal({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative w-full bg-card border border-border shadow-2xl overflow-hidden",
          "rounded-t-3xl sm:rounded-3xl",
          "sm:max-w-sm sm:mx-4 sm:mb-0",
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 pb-4">
          ...
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors ml-2 flex-shrink-0 mt-0.5">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Points info */}
        <div className="mx-5 mb-4 rounded-2xl bg-muted/60 border border-border p-4 flex items-center gap-4">
          <div className="flex-1">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Reward</p>
            <p className={cn("text-2xl font-black tabular-nums", isSubscribed ? "text-primary" : "text-muted-foreground")}>
              +{task.pts} pts
            </p>
            {task.daily && (
          ...
          {/* Completed count — only when subscribed */}
          {isSubscribed && (
            <div className="text-center">
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Completed</p>
              <p className={cn("text-xl font-black tabular-nums", task.done ? "text-primary" : "text-foreground")}>
                {task.done ? "1" : "0"}
              </p>
              <p className="text-[10px] text-muted-foreground">
                {task.daily ? "today" : "total"}
              </p>
            </div>
          )}
          ...
          {task.drawChances && task.drawChances > 0 ? (
            <div className="text-center">
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Draw</p>
              <p className="text-xl font-black text-yellow-500 tabular-nums">+{task.drawChances}</p>
              <p className="text-[10px] text-muted-foreground">
                {task.drawChances === 1 ? "chance" : "chances"}
              </p>
            </div>
          ) : null}
        </div>
        ...
        <div className="mx-5 mb-4 rounded-2xl border border-border bg-card p-4">
          <p className="text-xs font-semibold text-foreground mb-1">
            {isSubscribed ? "Tracking active" : "Subscribe to start earning"}
          </p>
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {isSubscribed
              ? "Your completed trades are being counted. Finish the action and points are awarded instantly — no manual claim needed."
              : "You must subscribe first. Completed trades only count toward points when you're opted in to this task."}
          </p>
        </div>

        {/* Actions */}
        <div className="px-5 pb-8 sm:pb-5 flex flex-col gap-2.5">
          <button
            onClick={onToggle}
            className={cn(
          ...
          {route && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm border border-border text-foreground hover:bg-muted transition-all"
            >
              {task.cta} <ArrowRight className="w-4 h-4" />
            </button>
          )}
        </div>
      </div>
...
function LuckyDrawModal({
  ...
  const typeIcons: Record<string, JSX.Element> = {
    Swap:      <Repeat2 className="w-5 h-5" />,
    Spot:      <TrendingUp className="w-5 h-5" />,
    Perpetual: <BarChart2 className="w-5 h-5" />,
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between p-5 pb-4">
          <div className="flex items-center gap-3">
            <div className={cn("w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0", typeColors[task.type])}>
              {typeIcons[task.type]}
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <p className="text-sm font-bold text-foreground leading-tight">{task.title}</p>
                <span className={cn("text-[9px] font-bold px-1.5 py-0.5 rounded-full", typeColors[task.type])}>{task.type}</span>
              </div>
              <p className="text-[11px] text-muted-foreground mt-0.5">Lucky Draw challenge</p>
            </div>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors ml-2 flex-shrink-0 mt-0.5">
            <X className="w-4 h-4" />
        ...
        <div className="mx-5 mb-4 rounded-2xl bg-muted/60 border border-border p-4 flex items-center gap-4">
          <div className="flex-1">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Points</p>
            <p className="text-2xl font-black text-primary tabular-nums">+{task.pts} pts</p>
          </div>
          <div className="text-center">
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Draw</p>
            <p className="text-xl font-black text-yellow-500 tabular-nums">+{task.chances}</p>
            <p className="text-[10px] text-muted-foreground">{task.chances === 1 ? "chance" : "chances"}</p>
          </div>
        </div>

        <div className="mx-5 mb-4 rounded-2xl border border-border bg-card p-4">
          <p className="text-xs font-semibold text-foreground mb-1">
            {isSubscribed ? "Tracking active" : "Subscribe to enter this challenge"}
          </p>
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            {isSubscribed
              ? "You're subscribed. Complete the trade threshold and your points + draw chances will be awarded automatically."
              : "Subscribe so Pexly tracks your volume for this challenge. Meet the threshold and earn points + Lucky Draw entries."}
          </p>
        </div>
        ...
        <div className="px-5 pb-8 sm:pb-5 flex flex-col gap-2.5">
          ...
          <button
            onClick={() => { onNavigate(task.route); onClose(); }}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm border border-border text-foreground hover:bg-muted transition-all"
          >
            {task.cta} <ArrowRight className="w-4 h-4" />
          </button>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
        {/* Completed-count pill — only visible when subscribed */}
        {isSubscribed && (
          <span className={cn(
            "text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 tabular-nums",
            task.done
              ? "bg-primary/15 text-primary"
              : "bg-muted text-muted-foreground",
          )}>
            {task.done ? <CheckCircle2 className="w-2.5 h-2.5" /> : null}
            {task.done ? "1" : "0"} completed
          </span>
        )}
        ...
        {task.drawChances != null && task.drawChances > 0 && (
...
function BadgeCard({ badge, expanded, onToggle }: { badge: Badge; expanded: boolean; onToggle: () => void }) {
  return (
    ...
    >
      ...
      <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0", rarityIconBg[badge.rarity], !badge.unlocked && "opacity-30")}>
        {badge.icon}
      </div>
...
// ─── Lucky Draw tasks data ────────────────────────────────────────────────────

const DRAW_TASKS = [
  { id: "draw-swap-100",    title: "Swap $100 or more",               type: "Swap",      chances: 1, pts: 50,  route: "/swap",       cta: "Swap now"     },
  { id: "draw-spot-500",    title: "Spot trade $500 or more",         type: "Spot",      chances: 1, pts: 75,  route: "/spot",       cta: "Trade spot"   },
  { id: "draw-perp-1000",   title: "Perpetual trade $1,000 or more",  type: "Perpetual", chances: 2, pts: 100, route: "/perpetual",  cta: "Trade perps"  },
  { id: "draw-swap-5000",   title: "Swap $5,000 or more",             type: "Swap",      chances: 2, pts: 200, route: "/swap",       cta: "Swap now"     },
  { id: "draw-spot-10000",  title: "Spot trade $10,000 or more",      type: "Spot",      chances: 2, pts: 300, route: "/spot",       cta: "Trade spot"   },
  { id: "draw-perp-15000",  title: "Perpetual trade $15,000 or more", type: "Perpetual", chances: 2, pts: 500, route: "/perpetual",  cta: "Trade perps"  },
];
...
export function RewardsEngagementSection({
  ...
  const [activeDrawModal, setActiveDrawModal] = useState<typeof DRAW_TASKS[number] | null>(null);

  const { isSubscribed, toggle } = useTaskSubscriptions();
    ...
    <>
      ...
      {activeTaskModal && (
        <TaskDetailModal
          ...
          isSubscribed={isSubscribed(activeTaskModal.id)}
          onToggle={() => toggle(activeTaskModal.id)}
      ...
      {activeDrawModal && (
        <LuckyDrawModal
          task={activeDrawModal}
          isSubscribed={isSubscribed(activeDrawModal.id)}
          onToggle={() => toggle(activeDrawModal.id)}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      Complete all 5 for <span className="text-primary font-semibold">25 pts/day</span>. Tap a task to subscribe and start tracking.
                    </p>
                  </div>
                </div>
              </div>
        ...
        {tab === "redeem" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-muted/60 rounded-xl px-4 py-3 border border-border">
              ...
              <button onClick={() => setTab("earn")} className="text-[11px] text-primary font-semibold flex items-center gap-0.5">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-muted-foreground bg-muted border border-border px-2 py-1 rounded-full">
            {DRAW_TASKS.filter(t => isSubscribed(t.id)).length} subscribed
          </span>
        </div>
        <div className="space-y-2">
          {DRAW_TASKS.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => setActiveDrawModal(item)}
                className="w-full flex items-center justify-between gap-4 p-4 rounded-2xl border border-border bg-card hover:bg-muted/50 transition-colors text-left"
              >
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={cn(
                    "text-[9px] font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1 transition-all",
                    subscribed
                      ? "bg-primary/10 text-primary border border-primary/20"
                      : "bg-muted text-muted-foreground border border-border",
                  )}>
                    {subscribed ? <><Bell className="w-2.5 h-2.5" /> On</> : "Join"}
                  </span>
                </div>
              </button>
            );
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "Lucky Spins",       desc: "Spin to win pts & prizes",         icon: "🎰", bg: "bg-purple-50 dark:bg-purple-500/10", border: "border-purple-200 dark:border-purple-500/20", btn: "text-purple-700 dark:text-purple-400", route: "/rewards"    },
            { label: "Price Predictions", desc: "Predict crypto prices, earn pts",  icon: "🔮", bg: "bg-blue-50 dark:bg-blue-500/10",   border: "border-blue-200 dark:border-blue-500/20",   btn: "text-blue-700 dark:text-blue-400",   route: "/prediction" },
            { label: "Swap & Earn",       desc: "Pts on every crypto swap",         icon: "🔄", bg: "bg-cyan-50 dark:bg-cyan-500/10",   border: "border-cyan-200 dark:border-cyan-500/20",   btn: "text-cyan-700 dark:text-cyan-400",   route: "/swap"       },
            { label: "Loyalty Badges",    desc: "Complete challenges for badges",   icon: "🏅", bg: "bg-amber-50 dark:bg-amber-500/10", border: "border-amber-200 dark:border-amber-500/20", btn: "text-amber-700 dark:text-amber-400", route: "/rewards"    },
            { label: "Cashback",          desc: "Get cashback on your trades",      icon: "🎁", bg: "bg-pink-50 dark:bg-pink-500/10",   border: "border-pink-200 dark:border-pink-500/20",   btn: "text-pink-700 dark:text-pink-400",   route: "/spot"       },
          ].map((item) => (
            <button
              key={item.label}
              onClick={() => navigate(item.route)}
              className={cn("rounded-2xl border p-4 flex flex-col gap-3 cursor-pointer transition-all hover:scale-[1.02] text-left", item.bg, item.border)}
            >
              ...
              <span className={cn("flex items-center gap-1 text-[10px] font-semibold mt-auto transition-all hover:gap-2", item.btn)}>
