    ...
    <div className="flex flex-col w-full bg-background">
      ...
      <div className="flex items-center gap-2 px-4 pt-3 pb-1">
        ...
        <div className="relative" ref={leverageRef}>
          <button
            ...
            className="px-3 py-1.5 text-xs font-semibold bg-secondary rounded text-foreground flex items-center gap-1"
          >
            {leverage}x
            {leverageOpen ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
          </button>
          {leverageOpen && (
            <div className="absolute z-50 mt-1 rounded border border-border bg-secondary shadow-lg min-w-[60px]">
      ...
      <div className="flex flex-col gap-3 p-4">
            ...
            <div className="relative">
              <div className="flex items-center w-full px-3 py-3 rounded border border-border bg-secondary overflow-hidden">
                <input
                  ...
                  className="flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground min-w-0"
                />
                <button className="text-xs text-trading-amber font-semibold mr-2 shrink-0">
                  BBO
                </button>
                <button
                  onClick={() => setPriceUnitDropdownOpen(!priceUnitDropdownOpen)}
                  className="flex items-center gap-1 text-sm text-foreground shrink-0"
                >
                  {priceUnit}
        ...
        <div className="flex flex-col gap-2 text-xs mt-1">
          ...
          {/* TP/SL + Hidden Order + Reduce-Only + GTC */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
            <label className="flex items-center gap-1.5 text-muted-foreground cursor-pointer">
              ...
              />
              <span className="border-b border-dashed border-muted-foreground">TP/SL</span>
            </label>
            {isLimit && (
              <label className="flex items-center gap-1.5 text-muted-foreground cursor-pointer">
                <input
            ...
            <label className="flex items-center gap-1.5 text-muted-foreground cursor-pointer">
              <input
                ...
                className="w-3.5 h-3.5 rounded border-border bg-secondary accent-trading-green"
              />
              <span className="border-b border-dashed border-muted-foreground">Reduce-Only</span>
            </label>
            {isLimit && (
              <div className="relative ml-auto" ref={tifRef}>
                <button
        ...
        {/* Connect button */}
        <button
          className={`w-full py-3 rounded-lg text-sm font-semibold mt-2 transition-opacity hover:opacity-90 ${
            side === "buy"
              ? "bg-trading-green text-foreground"
              : "bg-trading-red text-foreground"
          }`}
        ...
        <button className="w-full py-3 rounded-lg bg-trading-amber text-primary-foreground text-sm font-semibold">
          Connect
        </button>
      </div>
    </div>
