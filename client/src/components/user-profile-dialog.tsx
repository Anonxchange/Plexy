...
import { ThumbsUp, ThumbsDown, X, Award, Shield } from "lucide-react";
...
export function UserProfileDialog({
  ...
  const { profile, positiveCount, negativeCount, successRate, totalTrades, tradePartners, positiveFeedback, negativeFeedback, avgReleaseTime, tradesReleased } = userData;
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-semibold text-lg truncate">{profile?.username || "Unknown"}</h3>
                {profile?.country && (
        ...
        <div className="flex-1 overflow-y-auto">
          <Tabs defaultValue="trading" className="w-full">
            ...
            <TabsContent value="trading" className="p-6 space-y-6">
              <div>
                <h4 className="font-semibold mb-4 text-primary">Positive feedbacks</h4>
              ...
              <div>
                <h4 className="font-semibold mb-4 text-primary">Negative feedbacks</h4>
                <div className="flex items-center gap-2 mb-2">
              ...
              <div className="space-y-3 pt-4 border-t">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Trade partners</span>
                  <span className="font-semibold">{tradePartners}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Trades released</span>
                  <span className="font-semibold">{tradesReleased}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Trade success</span>
                  <span className="font-semibold">{successRate}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Avg. time to release</span>
                  <span className="font-semibold">{avgReleaseTime}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Trusted by</span>
                  <span className="font-semibold">{positiveCount} users</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Blocked by</span>
                  <span className="font-semibold">0 users</span>
                </div>
              </div>
            </TabsContent>
