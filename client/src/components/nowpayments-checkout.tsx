...
import { Loader2, Bitcoin, Copy, RefreshCw } from "lucide-react";
...
const NowPaymentsCheckout = ({
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      setPaymentData(data);
      toast.success("Payment created", { description: "Send crypto to the address shown below" });
    } catch (err: any) {
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      if (["finished", "confirmed"].includes(status.payment_status)) {
        ...
        toast.info("Payment pending", { description: `Status: ${status.payment_status}. Please wait.` });
      } else {
        toast.error("Payment status", { description: `Status: ${status.payment_status}` });
      }
    } catch {
