        ...
        <div className="bg-white border-b mb-6 rounded-t-xl overflow-hidden">
          <Tabs value={mainTab} onValueChange={setMainTab} className="w-full">
            <TabsList className="w-full justify-start h-14 bg-transparent p-0 px-6 gap-8">
              ...
              <TabsTrigger value="key-metrics" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none h-full px-0 font-bold text-base text-muted-foreground">Key Metrics</TabsTrigger>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="bg-white border-none shadow-sm rounded-xl">
            ...
            <CardContent className="space-y-4">
              {[{ name: "Privacy Coins", change: "16.55%", coin: "ROSE", coinPrice: "44.26%" }, { name: "AI", change: "12.80%", coin: "ROSE", coinPrice: "44.26%" }, { name: "Pantera Portfolio", change: "7.79%", coin: "ROSE", coinPrice: "44.26%" }, { name: "Polychain Portfolio", change: "7.37%", coin: "ROSE", coinPrice: "44.26%" }].map((sector, i) => (
                <div key={i} className="flex items-center justify-between group cursor-pointer">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-bold text-muted-foreground bg-muted/30 px-2 py-0.5 rounded uppercase tracking-tighter w-24 text-right">{sector.coin} <span className="text-green-500">{sector.coinPrice}</span></span>
                  </div>
                </div>
              ))}
            </CardContent>
              ...
              <TabsContent value="spot" className="mt-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-6 border-b">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {[{ pair: "LIT/USDT", price: "1.664", change: "-7.04%" }, { pair: "FOGO/USDT", price: "0.03142", change: "+2.11%" }, { pair: "FRAX/USDT", price: "1.1432", change: "-0.22%" }].map((item, i) => (
                        <div key={i} className="flex items-center justify-between text-sm font-bold">
                          <span className="text-slate-700">{item.pair}</span>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {[{ pair: "BTC/USDT", price: "92,731", change: "-0.01%" }, { pair: "ETH/USDT", price: "3,194.25", change: "-1.49%" }, { pair: "SOL/USDT", price: "133.56", change: "-0.48%" }].map((item, i) => (
                        <div key={i} className="flex items-center justify-between text-sm font-bold">
                          <span className="text-slate-700">{item.pair}</span>
                          <div className="flex gap-4"><span>{item.price}</span><span className={`${item.change.startsWith('-') ? 'text-red-500' : 'text-green-500'}`}>{item.change}</span></div>
                        </div>
